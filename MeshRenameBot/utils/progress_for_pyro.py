...
async def progress_for_pyrogram(
    ...
    if round(diff % time_out) == 0 or current == total:
        ...
        elapsed_time_str = human_readable_timedelta(elapsed_time)
        estimated_total_time_str = human_readable_timedelta(estimated_total_time)

        completed_str = await get_var("COMPLETED_STR")
        remaining_str = await get_var("REMAINING_STR")

        progress = "[{0}{1}] \nP: {2}%\n".format(
            ''.join([completed_str for _ in range(math.floor(percentage / 10))]),
            ''.join([remaining_str for _ in range(10 - math.floor(percentage / 10))]),
            round(percentage, 2))
